...
usage(void)
{
	fputs("usage: dmenu [-bfivPRS] [-l columns] [-l lines] [-h height] [-p prompt] [-fn font] [-m monitor]\n"
...
readxresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		/* Fuzzy match highlighting colors */
		if (XrmGetResource(xdb, "dmenu.backgroundHighlight", "*", &type, &xval))
			colors[SchemeNormHighlight][ColBg] = strdup(xval.addr);
		else
			colors[SchemeNormHighlight][ColBg] = strdup(colors[SchemeNormHighlight][ColBg]);
		if (XrmGetResource(xdb, "dmenu.foregroundHighlight", "*", &type, &xval))
			colors[SchemeNormHighlight][ColFg] = strdup(xval.addr);
		else
			colors[SchemeNormHighlight][ColFg] = strdup(colors[SchemeNormHighlight][ColFg]);
		if (XrmGetResource(xdb, "dmenu.selbackgroundHighlight", "*", &type, &xval))
			colors[SchemeSelHighlight][ColBg] = strdup(xval.addr);
		else
			colors[SchemeSelHighlight][ColBg] = strdup(colors[SchemeSelHighlight][ColBg]);
		if (XrmGetResource(xdb, "dmenu.selforegroundHighlight", "*", &type, &xval))
